...
class TestSegEquivariance(unittest.TestCase):
  ...
  def test_deeplab_resnet50_unpool(self):
    # seed everything
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    # Backbone
    backbone = get_backbone('ResNet50Custom')(
      input_shape=(),
      num_classes=21,
    ...
    print("[test_deeplab_resnet50_unpool] errval: ",errval)
    assert torch.allclose(y_shift,_y_shift)

  # DDACSegmentation + unpooling + antialias
  def test_deeplab_resnet50_unpool_lpf(self):
    # seed everything
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    with torch.no_grad():
      ...
      # Check error
      errval = torch.norm(_y_shift-y_shift)
    print("[test_deeplab_resnet50_unpool_lpf] errval: ",errval)
    assert torch.allclose(y_shift,_y_shift)

  # DDACSegmentation + unpooling + DDAC
  def test_deeplab_resnet50_unpool_ddac(self):
    # seed everything
    random.seed(0)
    np.random.seed(0)
    torch.manual_seed(0)
    ...
    with torch.no_grad():
      ...
      # Check error
      errval = torch.norm(_y_shift-y_shift)
    print("[test_deeplab_resnet50_unpool_ddac] errval: ",errval)
    assert torch.allclose(y_shift,_y_shift)
