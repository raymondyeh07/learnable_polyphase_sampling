...
class ImagenetteDataModule(pl.LightningDataModule):
  def __init__(self, batch_size, data_dir: str = './', aug_method='basic',
    ...
    self.dims = (3, 224, 224)
    self.num_classes = 10
    imagenette_mean = torch.tensor([0.485, 0.456, 0.406])
    imagenette_std = torch.tensor([0.229, 0.224, 0.225])
  ...
  def setup(self, stage=None):
    ...
    if stage == 'fit' or stage is None:
      if self.val_split is None:
        ...
        self.imagenette_train = ImageFolder(self.train_dir,
                                            transform=self.train_transform)
        # Don't apply augmentation to val. set.
        self.imagenette_val = ImageFolder(self.test_dir,
                                          transform=self.test_transform)
      else:
        # Split trainset
        imagenette_full = ImageFolder(self.train_dir,
                                      transform=self.train_transform)
        full_len = len(imagenette_full)
        val_len = int(full_len*self.val_split)
        train_len = full_len-val_len
        self.imagenette_train, self.imagenette_val = random_split(
             imagenette_full, [train_len, val_len])
        # Don't apply augmentation to val. set.
        self.imagenette_val.dataset = copy(imagenette_full)
        self.imagenette_val.dataset.transform = self.test_transform

    # Assign test dataset for use in dataloader(s)
    if stage == 'test' or stage is None:
      self.imagenette_test = ImageFolder(self.test_dir,
                                         transform=self.test_transform)

  def train_dataloader(self):
    return DataLoader(self.imagenette_train, batch_size=self.batch_size, shuffle=True, num_workers=CPU_COUNT)

  def val_dataloader(self):
    return DataLoader(self.imagenette_val, batch_size=self.batch_size, num_workers=CPU_COUNT)

  def test_dataloader(self):
    return DataLoader(self.imagenette_test, batch_size=self.batch_size, num_workers=CPU_COUNT)
